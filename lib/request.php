...
class request{
  ...
  protected function getMethod($arg){
    ...
    if(!$method){
      $this->abort($this::INVALID_REQUEST, 'Argument `method` not found.');
    } else {
      $this->method = $method;
    }
  }
    ...
    if(!in_array($_SERVER['REQUEST_METHOD'], ["GET", "COPY", "PURGE", "UNLOCK"])){
      if(array_key_exists('Content-Type', $this->clientHeaders)){
        ...
        switch($contentType){
          ...
          case 'text/xml':
            $this->data = xml2array($this->rawInput);
            $this->format = $this::FORMAT_XML;
            break;
          case 'application/x-yaml':
          case 'text/yaml':
            $this->data = Yaml::parse($this->rawInput);
          ...
          default:
            ...
            if(!($data === FALSE)){
              ...
              $this->abort($this::INVALID_REQUEST, 'false content-type');
            }
            break;
        }
      }
